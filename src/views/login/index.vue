<template>
  <div class="login-container">
    ...
    <!-- 右侧登录区 -->
    <div class="login-right" v-if="isWXLogin">
      <div class="login-form-wrapper">
        ...
        <div v-if="!isRegisterMode" class="social-login">
          <button type="button" class="google-btn" @click="wxlogin">
            <svg
              class="wechat-icon"
              viewBox="0 0 24 24"
              width="20"
    ...
    <!-- 扫码登录展示二维码 -->
    <WXQrcode v-else @login-success="onQrLoginSuccess" @back="onQrBack" />
  </div>
</template>
...
import WXQrcode from './components/qrcode.vue';
import { getWxQrCode } from '@/api/modules/wechat';
const { t } = useI18n();
const userStore = useUserStore();
const route = useRoute();
const isWXLogin = ref<boolean>(true);
...
const navigateAfterAuthSuccess = () => {
  ...
  window.open(loginRedirectTarget.value);
};
...
const wxlogin = () => {
  isMobile ? fetchUUID() : (isWXLogin.value = false);
};
...
const fetchUUID = async () => {
  try {
    ...
    window.open(
      `weixin://dl/business/?appid=wxd46d3f4025e9afc1&path=pages/index/index&query=${data.uuid}&env_version=develop`
    );
  } catch (err: any) {
...
const onQrLoginSuccess = async (_token: string) => {
  // 1. 同步 Pinia Store 的 token（qrcode 组件只写了 localStorage）
